...
def get_data_loaders(
    ...
    valid_data = datasets.ImageFolder(
        ...
        # YOUR CODE HERE: add the appropriate transform that you defined in
        # the data_transforms dictionary
        data_transforms.get("valid")
    )

    # obtain training indices that will be used for validation
    n_tot = len(train_data)
    indices = torch.randperm(n_tot) ## Returns a random permutation of integers from 0 ~ n_tot - 1
...
#     images = images.numpy() # convert images to numpy for display
    ...
    for idx in range(max_n):
        ...
        ax.set_title(class_names[idx].item())
